<footer class="container">
    <div class="flex py-5 flex-col md:flex-row items-center justify-between">
        <div class="text-sm">{!! cms('copyright') !!}</div>
        <a href="{{ home_url() }}" class="flex items-center justify-center text-accent-400 my-3 md:my-0">
            ...
            <span class="font-semibold text-lg ml-2">{{ cms('title', 'CoPlay') }}</span>
        </a>
        <p class="text-xs">
            {!! __('Powered by %s & %s', [
                '<a href="https://moyshan.netlify.app" class="underline hover:text-accent-500">Shahin</a>',
                '<a href="https://www.themoviedb.org/" class="underline hover:text-accent-500">TMDb API</a>',
            ]) !!}
        </p>
    </div>
