...
<div x-show="tabOpen === 'details'" class="text-primary-300">
    ...
    <?php if ($isTv && isset($video->number_of_episodes)): ?>
        <p class="text-primary-400 uppercase mt-4 mb-1"><?= _e('Total Episode/Season') ?></p>
        <p><?= $video->number_of_episodes ?>/<?= $video->number_of_seasons ?? 0 ?></p>
    <?php endif ?>
    <p class="text-primary-400 uppercase mt-4 mb-1"><?= _e('Synopsis') ?></p>
    <h3 class="text-xl font-semibold mb-2 text-primary-200"><?= $video->tagline ?></h3>
    <p><?= $video->overview ?></p>
    <?php if ($title != $original_title): ?>
        <p class="text-primary-400 uppercase mt-4 mb-1"><?= _e('Original Title') ?></p>
        <p><?= $original_title ?></p>
    <?php endif ?>
