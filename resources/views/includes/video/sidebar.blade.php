...
<div class="max-w-sm mx-auto mb-8 md:mb-0 md:w-4/12 lg:w-3/12">
    ...
    <div class="border border-primary-700 text-center md:text-left">
        ...
        <div class="mb-2 py-3 px-4 rounded-b text-primary-300 bg-primary-800 grid grid-cols-2 gap-2 text-center"
            x-data="manageWatchList( /** time: {{ microtime() }} */ )">
            <div>
                <button x-show="isWatchLater({{ $video->id }})"
                    x-on:click="removeFromWatchLater({{ $video->id }})">
                    ...
                    <span class="text-xs hidden md:block">{{ __('Remove') }}</span>
                </button>
                <button x-show="!isWatchLater({{ $video->id }})" x-on:click="addToWatchLater({{ $video->id }})"
                    ...
                    <span class="text-xs hidden md:block">{{ __('Watch Later') }}</span>
                </button>
            </div>
            @include('includes.video.parts.share')
        </div>
        @if (!empty($video->status) && strtolower($video->status) !== 'released')
            <p class="text-center px-4 uppercase text-primary-300 text-sm italic font-semibold">
                ({{ $video->status }})
            </p>
        @endif
        <div class="py-2 px-4 border-primary-800">
            <p class="mb-1 uppercase text-primary-400 font-semibold">{{ __('Rating') }}</p>
        ...
        <div class="border-t py-2 px-4 border-primary-800">
            <p class="mb-1 uppercase text-primary-400 font-semibold">{{ __('Genres') }}</p>
        ...
        <div class="border-t py-2 px-4 border-primary-800">
            <p class="mb-1 uppercase text-primary-400 font-semibold">{{ __('Languages') }}</p>
            <div class="text-primary-300 text-sm">
        ...
        <div class="border-t py-2 px-4 border-primary-800">
            <p class="mb-1 uppercase text-primary-400 font-semibold">{{ __('Countries') }}</p>
            <div class="text-primary-300 text-sm">
        ...
        <div class="border-t py-2 px-4 border-primary-800">
            <p class="mb-1 uppercase text-primary-400 font-semibold">{{ __('Companies') }}</p>
            <div class="text-primary-300 text-sm">
        ...
        @isset($video->networks)
            <div class="border-t py-2 px-4 border-primary-800">
                <p class="mb-1 uppercase text-primary-400 font-semibold">{{ __('Networks') }}</p>
                <div class="text-primary-300 text-sm">
                    {!! $video->networks(
                        fn($networks) => join(
                            ', ',
