...
@if (is_array($sliderItems) && !empty($sliderItems))
        ...
        <div class="swiper h-[40vh] md:h-[50vh] group mb-8 rounded-xs overflow-hidden relative">
            <div class="swiper-wrapper">
                @foreach ($sliderItems as $sliderItem)
                    @php
                        $isTv = isset($sliderItem['name'], $sliderItem['first_air_date']);
                    @endphp
                    <!-- Slider Item Start -->
                    <div x-cloak class="swiper-slide">
                        <!-- Slider Content START -->
                        <div
                            class="relative z-10 flex justify-center text-center md:justify-start md:text-left items-end h-full py-8 px-6 bg-linear-to-b from-primary-950/10 to-primary-950">
                            <div class="w-10/12 md:w-8/12">
                                <div data-swiper-parallax="-200" data-swiper-parallax-duration="325">
                                    <h2 class="text-xl md:text-2xl lg:text-3xl font-semibold mb-4">
                                        {{ $sliderItem['name'] ?? $sliderItem['title'] }}
                                    </h2>
                                    <div class="flex flex-wrap items-center justify-center md:justify-start gap-3">
                                        ...
                                        <div class="flex items-center gap-0.5 text-sm">
                                            ...
                                            <span>{{ carbon($sliderItem['first_air_date'] ?? $sliderItem['release_date'])->format('d M, Y') }}</span>
                                        </div>
                                    </div>
                                    <div class="flex flex-wrap justify-center md:justify-start gap-x-3 gap-y-1.5 mt-2">
                                        @foreach ($sliderItem['genres'] ?? [] as $id => $genre)
                                            <a href="{{ route_url('genre', [
                                                'type' => $isTv ? 'tv' : 'movie',
